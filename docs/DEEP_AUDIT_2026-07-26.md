...
These controls are fail-closed. `public/legal-content.js`, the legal templates and the DPIA remain drafts. The assessment start therefore stays unavailable until the controller data, exact published documents/hashes, approved non-draft localized bundle version and professional sign-offs are supplied. The NeuroMap repository still needs a separate reviewed server-side consumer/UI change; no browser-to-Cogniva dashboard call is permitted.
