...
The privacy notice, terms/consumer information and DPIA files in `docs/` are implementation templates marked **DRAFT**. Production assessment start and paid checkout must remain disabled while required identity fields or document hashes are missing, any legal/localized-content version is a draft, professional review is outstanding, or live provider end-to-end tests have not passed. The approved `LEGAL_CONTENT_VERSION` must exactly match the deployed 11-language consent bundle.

Pre-launch QA uses the explicit server-side `ASSESSMENT_ACCESS_MODE=preview` switch. In that mode the draft acknowledgement screens and free reflection can be tested, but responses/results remain in browser memory, the Cogniva analytics option is unavailable, no consent receipt is sent or stored, and checkout/email/AI remain disabled for new activity. Webflow-wide analytics and session-replay scripts remain a separate host-page audit responsibility. The default and every unknown mode resolve to `disabled`; `production` preserves the full approved-content, document-hash, PostgreSQL and signed-receipt gate. Use preview only on a noindex, preferably access-controlled staging surface, and never relabel draft text as approved.
